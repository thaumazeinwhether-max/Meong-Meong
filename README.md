...
![Meong Meong! おへや画面](assets/readme/hero_room.png)

白くふわふわした犬の「クルム」と同じ部屋で過ごす、PC向けの育成・コミュニケーションゲームです。ごはんや遊びを楽しめるほか、3種類のミニゲームで条件を満たすと、新しい遊びがサプライズで解放されます。
## ゲーム画面
### お散歩ジャンプ
![お散歩ジャンプ](assets/readme/walk_jump.png)
### 吠えるタイピング
![吠えるタイピング](assets/readme/typing_game.png)
### おやつキャッチ
![おやつキャッチ](assets/readme/catch_game.png)

## 開発背景

文系・IT未経験からゲーム開発の一連の工程を経験することを目的に、企画・仕様策定から実装、テスト、実プレイによる改善まで取り組みました。単に機能を実装するだけでなく、「クルムが画面の中で生きているように感じられること」を重視し、実プレイで見つかった操作性やゲームバランス、アニメーションの課題を繰り返し改善しています。
...
- 企画・仕様策定から実装、テスト、実プレイによる改善まで一連の開発工程を通して取り組みました。
- SQLiteでユーザーごとの自己ベストや解放状態を保存し、複数ユーザーの進捗が混ざらないように設計しました。
- クルムのアニメーションは、待機・歩行・ジャンプ・リアクションなどの状態を共通管理し、経過時間ベースで自然に切り替わるようにしました。
- 実プレイで「簡単すぎる」「動きが不自然」と感じた箇所を洗い出し、ミニゲームの難易度やアニメーションを調整しました。
- pytestで認証、SQLite、解放条件、ゲームルール、アニメーション、主要画面描画などを自動テストし、最終的に56件のテストを通過させました。

## AI活用

ChatGPT・Codex・ImageGenを、仕様整理、実装支援、コードレビュー、テスト観点の検討、画像素材の作成に活用しました。企画・仕様・採用する実装方針は自分で決定し、生成結果は実機で確認したうえで、不具合や操作性、ゲームバランス、アニメーションの課題を洗い出して修正しました。
